...
{
    [Header("API Settings")]
    [Tooltip("Flask /scene endpoint URL")]
    public string apiEndpoint = "http://localhost:5000/scene";

    [Header("Execution Control")]
    [Tooltip("Automatically run the first scan after Play")]
    public bool scanOnStart = true;

    [Tooltip("Periodic rescan interval in seconds. 0 = disabled\n600 = every 10 minutes")]
    ...
    [Tooltip("Tag name for MockCamera pivot nodes")]
    public string mockCameraTag = "MockCamera";

    [Header("Physics Layer Settings")]
    [Tooltip("Set to RoomLayer (the layer RoomArea BoxColliders are on)")]
    public LayerMask roomLayer;

    [Tooltip("Set to the layer furniture is on (snapshot cullingMask)")]
    public LayerMask furnitureLayer;

    [Header("Snapshot Camera")]
    [Tooltip("Dedicated snapshot camera, not the main scene camera\nRecommend placing under [_System] / SceneSyncManager")]
    public Camera snapshotCam;

    [Tooltip("Snapshot resolution in pixels (square, recommended 256-512)")]
    ...
    IEnumerator ScanRoutine()
    {
        if (proxiesRoot == null)
        {
            Debug.LogError("[SceneSync] proxiesRoot is not assigned — drag Proxies_Root into the Inspector");
            yield break;
        }
        if (snapshotCam == null)
        {
            Debug.LogError("[SceneSync] snapshotCam is not assigned — drag the snapshot camera into the Inspector");
            yield break;
        }
        ...
        if (pivots.Length == 0)
        {
            Debug.LogError($"[SceneSync] No GameObjects found with Tag='{mockCameraTag}' — " +
                           "set the Tag on CameraNode or check the mockCameraTag field");
            yield break;
        }
    ...
    IEnumerator PostToBackend(string json)
    {
        ...
        if (req.result == UnityWebRequest.Result.Success)
            ...
            Debug.LogWarning($"[SceneSync] Sync failed: {req.error}\n" +
                             $"URL={apiEndpoint} — is Flask running?");
    }
